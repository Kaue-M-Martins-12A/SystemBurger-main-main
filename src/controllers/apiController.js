...
exports.getAdminDashboard = async (req, res) => {
    try {
        ...
        let dateLimit = "";
        if (range === 'week') dateLimit = "datetime('now', '-7 days')";
        else if (range === 'month') dateLimit = "datetime('now', '-30 days')";
        else if (range === 'year') dateLimit = "datetime('now', '-1 year')";
        ...
        if (range === 'week') {
            salesQuery = `
                SELECT strftime('%d/%m', created_at) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
            ...
            salesQuery = `
                SELECT 'Semana ' || ((strftime('%d', created_at) - 1) / 7 + 1) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
                GROUP BY label ORDER BY created_at ASC
            `;
            ...
            salesQuery = `
                SELECT strftime('%m/%Y', created_at) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
                GROUP BY label ORDER BY created_at ASC
            `;
        ...
        const salesData = await db.all(salesQuery);
        ...
        const topByNiche = await db.all(`
            ...
            ${whereClause.replace('created_at', 'o.created_at')}
            GROUP BY p.category
        ...
        res.json({
            kpis: {
                vendas: totalVendasRange.t || 0,
                mediaPedidos: totalPedidosRange.c > 0 ? (totalVendasRange.t / totalPedidosRange.c) : 0,
                clientesAtivos: usersCount,
                entregasAndamento: entregasCount.c
            },
            charts: {
                sales: salesData,
                niches: topByNiche,
